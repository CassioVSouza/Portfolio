...
function ActiveMenu(nameClass){
    ...
    DeactivateAllMenus();
    document.querySelector(classMenu).classList.add(`ActiveMenu${window.currentTypeLight}`);
    return;
}
...
function DeactivateAllMenus(){
    menusList.forEach(element => {
        document.querySelector(element).classList.remove(`ActiveMenu${window.currentTypeLight}`);
    });
    return;
}
